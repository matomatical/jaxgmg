#!/bin/bash
#SBATCH --job-name=wspaper-pile-binom
#SBATCH --partition=high_priority
#SBATCH --time=5:00:00
#SBATCH --gpus-per-node=1
#SBATCH --chdir=/data/matthew_farrugia_roberts
#SBATCH --output=out/%A_%a-wspaper-pile-binom.stdout
#SBATCH --error=out/%A_%a-wspaper-pile-binom.stderr
...
flags=(
    ...
    --wandb-project wspaper-pile-binom
    --env-size 15
    --max-cheese-radius 0
    ...
    for k in {0..7}; do
        ...
        if [ $SLURM_ARRAY_TASK_ID -eq $((j + 0)) ]; then
            jaxgmg train pile "${flags[@]}" \
                ...
                --no-chain-mutate --mutate-cheese-on-pile;
